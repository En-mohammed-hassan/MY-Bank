...
def _validate_token_audience(claims: dict[str, Any]) -> None:
    ...
    # customer-app tokens + staff bank-web tokens (aud=account) + bank-web
    allowed = {settings.keycloak_public_client_id, "account", "bank-web"}
...
def require_customer_roles(*allowed_roles: CustomerRole):
...
def require_staff_roles(*allowed_roles: StaffRole):
    allowed = {role.value for role in allowed_roles}
